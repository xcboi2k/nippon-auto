        ...
        <View className="relative flex-1 justify-start pb-[20px] w-full">
            ...
            <ScrollView>
                <View className="w-full px-[30px] flex flex-col items-center mb-[15px]">
                    ...
                    {numObjects > 1 ? (
                        <Text className="text-[18px] mb-[8px]">
                            Based on {numObjects} reviews
                        </Text>
                    ) : (
                        <Text className="text-[18px] mb-[8px]">
                            Based on {numObjects} review
                        </Text>
                    )}
                </View>
                <View className="flex-row justify-center mb-4 border-b border-gray-200">
                    ...
                    >
                        <Text
                            className={`font-medium ${activeTab === 'from' ? 'text-[#234791]' : 'text-gray-500'}`}
                        >
                            Reviews From Buyer
                        </Text>
                    </TouchableOpacity>
                    ...
                    >
                        <Text
                            className={`font-medium ${activeTab === 'to' ? 'text-[#234791]' : 'text-gray-500'}`}
                        >
                            Reviews To Seller
                        </Text>
                    </TouchableOpacity>
                ...
                <View className="w-full px-[30px] flex flex-col items-center mb-[15px]">
                    {filteredReviews ? (
                        filteredReviews.map((item, index) => (
                            <View
                                className="w-full rounded-lg bg-[#F4F6F8] mb-4 p-4"
                                key={index}
                            >
                                <View className="flex-row items-center justify-between mb-2">
                                    <Text className="text-lg font-bold text-[#153A56]">
                                        {item.user_name}
                                    </Text>
                                ...
                                <Text className="text-xs mb-[15px]">
